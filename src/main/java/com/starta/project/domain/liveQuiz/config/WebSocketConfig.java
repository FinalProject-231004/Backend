...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    public void configureMessageBroker(MessageBrokerRegistry config) {
        config.enableSimpleBroker("/api"); // 메시지 브로커에 대한 Prefix 설정
        config.setApplicationDestinationPrefixes("/api"); // 클라이언트에서 메시지 송신 시 prefix 설정
    }
    // topic으로 시작하는 메시지가 message-handling methods로 라우팅 되어야 한다는 것을 정의
    @Override
    public void registerStompEndpoints(StompEndpointRegistry registry) {
        registry.addEndpoint("/liveQuizChatRoom");
    }
}
